...
class LaundryRoom(metaclass=SingletonMeta):
    ...
    def check_occupany(self):
        now = datetime.datetime.now()

        D1Time = (now - (self.D1[2] + datetime.timedelta(minutes=self.D1[3]))).total_seconds()
        D1Info = (self.D1[0], self.D1[1], "{} minutes left".format(0 if D1Time else int(D1Time/-60)))

        D2Time = (now - (self.D2[2] + datetime.timedelta(minutes=self.D2[3]))).total_seconds()
        D2Info = (self.D2[0], self.D2[1], "{} minutes left".format(0 if D2Time else int(D2Time/-60)))

        D3Time = (now - (self.D3[2] + datetime.timedelta(minutes=self.D3[3]))).total_seconds()
        D3Info = (self.D3[0], self.D3[1], "{} minutes left".format(0 if D3Time else int(D3Time/-60)))

        W1Time = (now - (self.W1[2] + datetime.timedelta(minutes=self.W1[3]))).total_seconds()
        W1Info = (self.W1[0], self.W1[1], "{} minutes left".format(0 if W1Time else int(W1Time/-60)))

        W2Time = (now - (self.W2[2] + datetime.timedelta(minutes=self.W2[3]))).total_seconds()
        W2Info = (self.W2[0], self.W2[1], "{} minutes left".format(0 if W2Time else int(W2Time/-60)))
...
async def check_callback(event: slack_util.Event, match: Match) -> None:
    laundry = LaundryRoom()
    tempResult = "-     D1\nD2    W1\nD3    W2\nD1: {}\nD2: {}\nD3: {}\nW1: {}\nW2: {}"
    occupany = laundry.check_occupany()
